...
def get_dados_formulario(
    ...
    if caracteristica_raw:
        ...
        itens_memoria = [
            ...
            for i in (memoria if isinstance(memoria, list) else [])
        ]

        itens_mao_obra = [
            {
                "descricao":      str(i.get("descricao",      "")),
                # suporta tanto "visitas" (novo) quanto "qtd" (legado)
                "visitas":        float(i.get("visitas", i.get("qtd", 0))),
                "valor_unitario": float(i.get("valor_unitario", 0)),
                "subtotal":       float(i.get("subtotal",       0)),
            }
            for i in (mao_obra if isinstance(mao_obra, list) else [])
        ]

        caracteristica = CaracteristicaFormulario(
            id=caracteristica_raw.id,
            justificativa=caracteristica_raw.justificativa,
            entidade_elaboracao=getattr(caracteristica_raw, "entidade_elaboracao", None),
            texto_entidade_responsavel=getattr(caracteristica_raw, "texto_entidade_responsavel", None),
            # ← nomes corretos que o Vue lê: itens_investimento / itens_mao_obra
            itens_investimento=itens_memoria,
            itens_mao_obra=itens_mao_obra,
        )

    # Submissão mais recente (se houver)
    submissao = session.exec(
        select(Submissao)
        .where(Submissao.produtor_id == produtor_id)
        .where(Submissao.fomento_id  == fomento_id)
        .order_by(Submissao.id.desc())   # ← evita depender de criado_em
    ).first()

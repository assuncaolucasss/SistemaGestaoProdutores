    ...
    <div v-if="produtor">
      <div class="flex items-center justify-between mb-5">
        ...
        <div class="flex gap-2">
          <button v-if="isAdmin && !editando" @click="iniciarEdicao" class="flex items-center gap-1.5 bg-primary-600 hover:bg-primary-700 text-white text-sm px-4 py-2 rounded-lg border-none cursor-pointer transition-colors">
            <Pencil class="w-3.5 h-3.5" /> Editar
          </button>
          <template v-if="editando">
            <button @click="salvar" :disabled="salvando" class="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 disabled:opacity-60 text-white text-sm px-4 py-2 rounded-lg border-none cursor-pointer transition-colors">
              <Loader2 v-if="salvando" class="w-3.5 h-3.5 animate-spin" />
              <Save v-else class="w-3.5 h-3.5" />
              {{ salvando ? 'Salvando...' : 'Salvar' }}
            </button>
            <button @click="cancelarEdicao" class="flex items-center gap-1.5 bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 text-sm px-4 py-2 rounded-lg cursor-pointer transition-colors">
      ...
      <div v-if="erroSalvar" class="flex items-center gap-2 text-red-600 text-xs bg-red-50 border border-red-200 rounded-lg px-4 py-2.5 mb-4">
        <AlertCircle class="w-4 h-4" /> {{ erroSalvar }}
      </div>
      <div v-if="sucessoSalvar" class="flex items-center gap-2 text-green-700 text-xs bg-green-50 border border-green-200 rounded-lg px-4 py-2.5 mb-4">
      ...
      <div class="bg-white border border-gray-200 rounded-2xl p-7 mb-8 text-sm">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          <div>
            ...
            <select v-if="editando" v-model="form.assentamento" class="w-full px-3 py-2 border border-primary-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-600 uppercase">
              <option value="">—</option>
      ...
      <div v-else-if="fomentos.length === 0" class="flex flex-col items-center justify-center text-gray-400 py-8 gap-2 border border-dashed border-gray-200 rounded-xl mb-8">

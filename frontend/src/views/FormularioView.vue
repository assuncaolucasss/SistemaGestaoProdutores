  ...
  <div class="max-w-4xl mx-auto px-6 py-10">
    ...
    <div v-else-if="produtor && fomento" class="bg-white border border-gray-200 rounded-2xl p-8">
      ...
      <div class="flex flex-col gap-4 mb-6">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          <div>
            ...
            >
              ...
              <option
                v-for="si in subclassesDaClasse"
                :key="si.subclasse.id"
                :value="si.subclasse.id"
              >
                {{ si.subclasse.nome }}
              </option>
            </select>
        ...
        <div
          v-else-if="caracteristicaCarregada"
        ...
        <div v-if="eFomentoJovem" class="bg-amber-50 border border-amber-300 rounded-xl p-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                v-model="form.segundo_beneficiario_nome"
            ...
            <div>
              ...
              <input
                v-model="form.segundo_beneficiario_cpf"
        ...
        <div>
          ...
          <input
            v-model="form.entidade_elaboracao"
        ...
        <div>
          <textarea
            v-model="form.texto_entidade_responsavel"
            @input="form.texto_entidade_responsavel = form.texto_entidade_responsavel.toUpperCase()"
            rows="3"
            placeholder="TEXTO ADICIONAL SOBRE A ENTIDADE RESPONSÁVEL..."
          ...
          <tbody>
            <tr v-for="(item, i) in form.itens_investimento" :key="i">
              ...
              <td class="border-none p-1 text-center">
                <button
                  @click="form.itens_investimento.splice(i, 1)"
      ...
      <button
        @click="adicionarItem('investimento')"
            ...
            <tr v-for="(item, i) in form.itens_mao_obra" :key="i">
              ...
              <td class="border border-gray-200 p-1">
                <input
                  ...
                  type="number"
                  @input="calcularSubtotalMao(item)"
                  class="w-full px-2 py-1.5 text-sm border-none outline-none text-center"
                />
              </td>
              <td class="border border-gray-200 p-1">
              ...
              <td class="border-none p-1 text-center">
                <button
                  @click="form.itens_mao_obra.splice(i, 1)"
                  class="text-red-400 hover:text-red-600 bg-transparent border-none cursor-pointer p-1"
                >
                  <X class="w-3.5 h-3.5" />
                </button>
...
function adicionarItem(tipo) {
  if (tipo === 'investimento') {
    form.value.itens_investimento.push({
      discriminacao: '',
      quantidade: 0,
      valor_unitario: 0,
      subtotal: 0,
    ...
    form.value.itens_mao_obra.push({
      descricao: '',
      qtd: 0,
      valor_unitario: 0,
      subtotal: 0,
...
function gerarEntidadeResponsavelPDF() {
  ...
  return `
    <div style="margin-bottom:20px">
      <h2 style="color:#1a6b3c;font-size:13px;margin-bottom:6px;text-transform:uppercase">
        Entidade responsável pela elaboração/execução/acompanhamento deste projeto
      </h2>
      <p style="font-size:13px;font-weight:bold;text-transform:uppercase;margin:0 0 4px 0">
        ${u(form.value.entidade_elaboracao)}
      </p>
      ${textoExtra}
    </div>
  `
...
function gerarJustificativa() {
  ...
  return `
    <div style="margin-bottom:20px">
      <h2 style="color:#1a6b3c;font-size:13px;margin-bottom:8px;text-transform:uppercase">
        Justificativa do Projeto Produtivo
      </h2>
      <p style="font-size:11px;line-height:1.7;white-space:pre-wrap;padding:10px;border:1px solid #ccc;border-radius:4px;text-transform:uppercase">
        ${u(form.value.justificativa)}
      </p>
    </div>
  `
}
...
async function emitirPDF() {
  ...
  try {
    ...
    const assinaturas = eFomentoJovem.value
      ? gerarAssinaturasJovem()
      : gerarAssinaturasNormal()
    ...
    const html = `
      <div style="font-family:Arial,sans-serif;padding:40px;background:white;color:#000;width:794px;box-sizing:border-box">
        ...
        <div style="background:#e8f5e9;padding:14px 18px;border-radius:6px;text-align:right;margin-bottom:40px;border:2px solid #1a6b3c">
          <strong style="color:#1a6b3c;font-size:14px">
            TOTAL FINAL: R$ ${totalFinal.value.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}
          </strong>
        </div>

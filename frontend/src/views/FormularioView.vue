  ...
  <div class="max-w-4xl mx-auto px-6 py-10">
    ...
    <div v-if="produtor && fomento" class="bg-white border border-gray-200 rounded-2xl p-8">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm mb-6">
        ...
        <div>
          <span class="text-gray-400 text-xs">Assentamento</span>
          <p class="font-medium text-gray-700 uppercase">{{ produtor.assentamento }}</p>
        </div>
        <div>
          <span class="text-gray-400 text-xs">Lote</span>
          <p class="font-medium text-gray-700 uppercase">{{ produtor.lote }}</p>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-4 mb-6">
        ...
        <div v-if="eFomentoJovem" class="bg-amber-50 border border-amber-300 rounded-xl p-5">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input v-model="form.segundo_beneficiario_cpf" placeholder="000.000.000-00" maxlength="14" class="w-full px-3 py-2.5 border border-amber-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-amber-400" />
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <textarea v-model="form.justificativa" rows="4" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-600 resize-y" />
        </div>
            ...
            <tbody>
              <tr v-for="(item, i) in form.itens_investimento" :key="i">
                <td class="border border-gray-200 p-1">
                  <input v-model="item.discriminacao" class="w-full px-2 py-1.5 text-sm border-none outline-none" />
                </td>
                <td class="border border-gray-200 p-1">
                  <input v-model.number="item.quantidade" type="number" @input="calcularSubtotal(item)" class="w-full px-2 py-1.5 text-sm border-none outline-none text-center" />
                </td>
                <td class="border border-gray-200 p-1">
                  <input v-model.number="item.valor_unitario" type="number" @input="calcularSubtotal(item)" class="w-full px-2 py-1.5 text-sm border-none outline-none text-center" />
                </td>
                ...
                <td class="border-none p-1 text-center">
                  <button @click="form.itens_investimento.splice(i, 1)" class="text-red-400 hover:text-red-600 bg-transparent border-none cursor-pointer p-1">
                    <X class="w-3.5 h-3.5" />
                  </button>
                </td>
              </tr>
        ...
        <button @click="adicionarItem('investimento')" class="flex items-center gap-1.5 bg-primary-50 hover:bg-primary-100 border border-primary-600 text-primary-600 text-xs px-3 py-2 rounded-lg cursor-pointer mb-8 transition-colors">
          ...
          <table class="w-full text-sm border-collapse">
            ...
            <tbody>
              <tr v-for="(item, i) in form.itens_mao_obra" :key="i">
                <td class="border border-gray-200 p-1">
                  <input v-model="item.descricao" class="w-full px-2 py-1.5 text-sm border-none outline-none" />
                </td>
                <td class="border border-gray-200 p-1">
                  <input v-model.number="item.visitas" type="number" @input="calcularSubtotalMao(item)" class="w-full px-2 py-1.5 text-sm border-none outline-none text-center" />
                </td>
                <td class="border border-gray-200 p-1">
                  <input v-model.number="item.valor_unitario" type="number" @input="calcularSubtotalMao(item)" class="w-full px-2 py-1.5 text-sm border-none outline-none text-center" />
                </td>
                <td class="border border-gray-200 p-1 text-center text-primary-600 font-semibold text-xs">
                  R$ {{ (item.subtotal || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }}
                </td>
                <td class="border-none p-1 text-center">
                  <button @click="form.itens_mao_obra.splice(i, 1)" class="text-red-400 hover:text-red-600 bg-transparent border-none cursor-pointer p-1">
                    <X class="w-3.5 h-3.5" />
                  </button>
                </td>
              </tr>
        ...
        <button @click="adicionarItem('mao_obra')" class="flex items-center gap-1.5 bg-primary-50 hover:bg-primary-100 border border-primary-600 text-primary-600 text-xs px-3 py-2 rounded-lg cursor-pointer mb-8 transition-colors">
          <Plus class="w-3.5 h-3.5" /> Adicionar item
        </button>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
          ...
          <div>
            ...
            <input v-model="form.data_assinatura" type="date" class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-600" />
          </div>
        </div>

        <!-- Total -->
        <div class="bg-primary-50 border border-primary-600 rounded-xl px-6 py-4 text-right mb-6">
          <span class="text-primary-600 font-bold text-base">TOTAL FINAL: R$ {{ totalFinal.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }}</span>
        </div>
...
async function salvar() {
  ...
  try {
    await api.post('/submissoes', {
      ...
      itens_investimento: form.value.itens_investimento.length > 0 ? form.value.itens_investimento : null,
      itens_mao_obra: form.value.itens_mao_obra.length > 0 ? form.value.itens_mao_obra : null,
    })
    sucesso.value = true
    setTimeout(() => {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; padding: 40px; background: white; color: #000; width: 794px; box-sizing: border-box;">
        <div style="text-align: center; border-bottom: 2px solid #1a6b3c; padding-bottom: 16px; margin-bottom: 24px;">
          <p style="color: #999; margin: 0 0 4px 0; font-size: 11px; text-transform: uppercase; letter-spacing: 1px;">${produtor.value?.codigo_beneficiario} | Produtor ID: ${produtorId}</p>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 11px;">
          ...
          <tbody>
            ${form.value.itens_investimento.map(item => `
              <tr>
        ...
        <div style="background: #e8f5e9; padding: 14px 18px; border-radius: 6px; text-align: right; margin-bottom: 40px; border: 2px solid #1a6b3c;">
          <strong style="color: #1a6b3c; font-size: 14px;">TOTAL FINAL: R$ {{ totalFinal.toLocaleString('pt-BR', { minimumFractionDigits: 2 }) }}</strong>
        </div>
      </div>
    `
